...
function buildMaterialQuery(materialId) {
  const query = [{ materialId }, { _id: materialId }];
  if (ObjectId.isValid(materialId)) {
    query.push({ _id: new ObjectId(materialId) });
  }
  return { $or: query };
}
...
export async function GET(request) {
  return withApiHardening(
    request,
    { route: "download", rateLimit: { limit: 25, windowMs: 60_000 } },
    async () => {
      try {
        ...
        if (!materialId) {
          throw new ProtectedDownloadError(400, "invalid_material_id", "Missing material identifier");
        }

        if (accessToken) {
          const payload = await verifySignedDownload(accessToken, materialId);
          const material = await db.collection("materials").findOne(buildMaterialQuery(materialId));
