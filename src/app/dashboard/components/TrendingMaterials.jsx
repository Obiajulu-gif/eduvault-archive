	...
	return (
		<div className="p-6 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-sm">
			<div className="flex justify-between items-center mb-6">
				<h3 className="font-bold text-gray-900 dark:text-gray-100">Trending Materials</h3>
					...
					<div className="space-y-4">
						{[1, 2, 3].map(i => (
							<div key={i} className="flex gap-4 items-start">
								<div className="w-16 h-20 bg-gray-100 dark:bg-gray-800 rounded-md animate-pulse" />
								<div className="flex-1 space-y-2">
									<div className="h-3 bg-gray-100 dark:bg-gray-800 rounded w-full animate-pulse" />
									<div className="h-2 bg-gray-100 dark:bg-gray-800 rounded w-1/2 animate-pulse" />
								</div>
							</div>
						))}
					</div>
					...
					<div className="space-y-4">
						{data.items?.slice(0, 3).map((item, i) => (
							<Link
								...
								key={item._id || i}
								className="flex gap-4 items-start group hover:bg-gray-50 dark:hover:bg-gray-800 p-2 -mx-2 rounded-lg transition-colors cursor-pointer block"
							>
								...
								<div className="flex-1 min-w-0 py-1">
									<h4 className="font-semibold text-sm text-gray-900 dark:text-gray-100 truncate mb-1">{item.title}</h4>
									<p className="text-xs text-gray-500 dark:text-gray-400 mb-2 truncate">by {item.author || item.creator || "Anonymous"}</p>
									<div className="flex justify-between items-center text-xs">
										<span className="flex items-center text-gray-500 gap-1">
											<FaHeart className="w-3 h-3 text-rose-400" />
											{item.likes || 0}
										</span>
										<span className="font-bold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-950/40 px-2 py-0.5 rounded-full">
