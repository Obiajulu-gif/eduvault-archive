...
export default function DiscoverMaterials() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-white to-blue-50 px-6 py-20 md:px-16">
      <div
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(to_bottom,rgba(0,0,0,0.03)_1px,transparent_1px)] bg-[size:50px_50px] opacity-30"
      />
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        variants={fadeUp}
        className="relative mb-10 flex flex-col justify-between md:flex-row md:items-center z-10"
      >
      ...
      >
        {loading
          ? Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-[320px] rounded-2xl border border-gray-200 bg-gray-100 p-4 animate-pulse"
              >
